...
class AutonomousNav():
	def __init__(self):
		...
		self.target_position_tolerance=0.1 #acceptable distance to the goal to declare therobot has arrived to it [m]
	...
	def get_closest_object(self, lidar_msg):
		...
		closest_angle = min_angle + min_idx * lidar_msg.angle_increment # + new_angle_min
	...
	def compute_gtg_control(self, x_target, y_target, x_robot, y_robot, theta_robot):
		...
		e_theta = self.theta_target-theta_robot
