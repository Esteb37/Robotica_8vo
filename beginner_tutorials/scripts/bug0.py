...
class AutonomousNav():
			...
			if self.lidar_received:
				...
				if self.current_state == 'Stop':
					...
					if self.at_goal():
						...
						angle_diff = self.normalize_angle(closest_angle - self.theta_gtg)
	...
	def get_closest_object(self, lidar_msg):
		...
		cropped_ranges = lidar_msg.ranges
		min_idx = np.argmin(cropped_ranges)
		closest_range = cropped_ranges[min_idx]
		closest_angle = min_angle + min_idx * lidar_msg.angle_increment
	...
	def compute_gtg_control(self, x_target, y_target, x_robot, y_robot, theta_robot):
		...
		self.theta_target = np.arctan2(y_target - y_robot, x_target - x_robot)
		e_theta = self.normalize_angle(self.theta_target - theta_robot)
		# Compute the robot's angular speed
		kw = kwmax * (1 - np.exp(-aw * e_theta**2)) / abs(e_theta)
		w = kw * e_theta
		self.theta_gtg = e_theta
	...
	def compute_fw_control(self, closest_angle, clockwise):
		...
		self.theta_AO = self.normalize_angle(self.theta_AO)
